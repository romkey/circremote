...
class Config:
    ...
    def find_command_in_search_paths(self, command_name):
        ...
        for search_path in self.search_paths:
            ...
            if command_path.exists() and command_path.is_dir():
                ...
                if code_file.exists():
                    self.debug(
                        f"Found command '{command_name}' in search path: {search_path}"
                    )
                    return command_path
        ...
        if user_commands_dir.exists():
            ...
            if command_path.exists() and command_path.is_dir():
                ...
                if code_file.exists():
                    self.debug(
                        f"Found command '{command_name}' in user commands: {user_commands_dir}"
                    )
                    return command_path
    ...
    def load_config(self):
        ...
        try:
            with open(self.config_path, "r", encoding="utf-8") as f:
                ...
                # Load devices
                if "devices" in config_data and isinstance(
                    config_data["devices"], list
                ):
                    ...
                    for device in config_data["devices"]:
                        ...
                        self.debug(
                            f"Added device: {device['name']} -> {device['device']}"
                        )
                else:
                    self.debug("No 'devices' array found in config")

                # Load command aliases
                if "command_aliases" in config_data and isinstance(
                    config_data["command_aliases"], list
                ):
                    self.debug(
                        f"Found {len(config_data['command_aliases'])} command aliases in config"
                    )
                    for alias in config_data["command_aliases"]:
                        ...
                        self.debug(
                            f"Added command alias: {alias['name']} -> {alias['command']}"
                        )
                else:
                    self.debug("No 'command_aliases' array found in config")

                # Load search paths
                if "search_paths" in config_data and isinstance(
                    config_data["search_paths"], list
                ):
                    self.debug(
                        f"Found {len(config_data['search_paths'])} search paths in config"
                    )
                    for search_path in config_data["search_paths"]:
                        ...
                        if path.exists() and path.is_dir():
                            ...
                            print(
                                f"Warning: Search path '{search_path}' does not exist or is not accessible"
                            )
                else:
                ...
                if "variable_defaults" in config_data and isinstance(
                    config_data["variable_defaults"], dict
                ):
                    self.variable_defaults = config_data["variable_defaults"].copy()
                    self.debug(
                        f"Found {len(self.variable_defaults)} variable defaults in config: {list(self.variable_defaults.keys())}"
                    )
                else:
            ...
            print(
                f"❌ Error: Config file {self.config_path} contains invalid JSON: {e}"
            )
            import sys
    ...
    def check_file_permissions(self):
        ...
        try:
            ...
            if stat_info.st_mode & stat.S_IROTH or stat_info.st_mode & stat.S_IWOTH:
                print(
                    f"Warning: Config file {self.config_path} has world access permissions"
                )
                print(f"  Owner: {file_owner}")
                print(f"  Current user: {current_user}")
                print(f"  Permissions: {oct(stat_info.st_mode)}")
                print(
                    f"  This is a security risk. Consider running: chmod 600 {self.config_path}"
                )
                print()
        except Exception as e:
    ...
    def validate_command_alias_config(self, alias):
        ...
        if "name" not in alias or not isinstance(alias["name"], str):
            raise ValueError(
                "Command alias configuration must have a 'name' string field"
            )

        if "command" not in alias or not isinstance(alias["command"], str):
            raise ValueError(
                "Command alias configuration must have a 'command' string field"
            )
