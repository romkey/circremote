...
class TestConfig:
    ...
    def test_find_device_and_alias(self):
        ...
        config.command_aliases = {"foo": "BME280", "bar": "SHT30"}
    ...
    def test_find_device_by_path(self):
        ...
        assert config.find_device_by_path("/dev/ttyUSB1") == {
            "name": "dev2",
            "device": "/dev/ttyUSB1",
        }
    ...
    def test_validate_device_config(self):
        ...
        with pytest.raises(ValueError):
            config.validate_device_config(
                {"name": "dev", "device": "/dev/ttyUSB0", "friendly_name": 123}
            )
        # Wrong type for password
        with pytest.raises(ValueError):
            config.validate_device_config(
                {"name": "dev", "device": "/dev/ttyUSB0", "password": 123}
            )
    ...
    def test_load_config_file_with_circup_path(self, tmp_path):
        ...
        config_data = {"circup": "/usr/local/bin/circup"}
        config_dir = tmp_path / ".circremote"
        config_dir.mkdir()
        config_path = config_dir / "config.json"
        config_path.write_text(json.dumps(config_data))
    ...
    def test_get_circup_path_from_system_path(self):
        ...
        with patch("os.path.exists", return_value=False), patch(
            "shutil.which", return_value="/usr/bin/circup"
        ):
